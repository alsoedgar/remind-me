...
export type AssistantRequestRewriteKind =
  'structured-list' | 'colloquial-delete' | 'colloquial-move' | 'next-query' | 'calendar-reflection'
...
export function hasCalendarRequestSignal(value: string): boolean {
  return /\b(?:add|agenda|appointment|availability|available|book|calendar|cancel|change|class|complete|conflict|copy|course|create|delete|duplicate|event|find|meeting|modify|move|plans?|remind|reminder|remove|rename|repeat|reschedule|schedule|search|shift|show|today|tomorrow|tmr|tmrw|tmw|update|weekend|monday|tuesday|wednesday|thursday|friday|saturday|sunday|jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\b|\b(?:am|will|would|could)\s+i\s+(?:be\s+)?(?:free|busy)\b|\bwhat (?:do|did) i have\b|\bwhat(?:'s|s| is) (?:on|coming up|next|tomorrow|tmr|tmrw|tmw|today)\b|^(?:when|where)\s+is\b|\b(?:at|around|before|after|from|between|until|by)\s+\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)?\b|\b\d{1,4}[/-]\d{1,2}(?:[/-]\d{1,4})?\b/iu.test(
    value
  )
}
