...
function documentFallbackPrompt(request) {
  ...
  return (
    ...
    'Find complete calendar items that the built-in reader left ungrouped. Use only the exact quoted strings after BLOCK_ID= from this one-page window; never use row numbers or shortened IDs. For each unclaimed time block, connect the nearest matching item-name block, explicit date or date-range block, optional room block, and optional weekday/repeat block. A term date range plus weekdays plus a fixed time is a complete repeating class and must be grouped. A block marked claimed may provide nearby context, but every returned group needs an unclaimed date or time block. Keep each event, class component, or reminder separate. Use descriptionBlockIds only for actual notes, otherwise use an empty array. Use null for a missing optional block. The caller has already verified that this window contains at least one plausible complete combination, so return at least one source-supported group.\n' +
