...
verified ResponsePlan
  -> one of 22 protected speech acts + exact fact signature + style
...
The 22 acts distinguish proposals, four kinds of mutation receipt, availability, schedule summaries, next items, item details, empty schedules, ordinary conversation, approved memory, undo, rejected proposals, clarification, conflicts, unsupported requests, and errors. Phase 5 adds explicit conversational-clarification, runtime-unavailable, offline-fact-limit, and policy-boundary acts. This prevents benign contextual or general chat from borrowing policy wording such as “safe action,” while keeping genuine policy refusals distinct.

## Model and data

Version 0.4 has 32,440,848 logical parameters across three sparse INT8 heads and 61,440 deterministic feature buckets. The lead/body/close inventories contain 110, 308, and 110 options. The 32,440,320-byte table occupies 30.9 MiB when decompressed; the JSON plus reproducibly gzipped weights occupy 272,708 bytes because structured examples activate only a small fraction of the logical table. The compression ratio is disclosed rather than treating sparse logical capacity as dense learned information.

`ml/remindspeak/pipeline.py` creates a 40,000-example program-generated base corpus across all 22 acts, then adds 1,650 replay rows from 55 accepted Qwen rankings over already-valid candidates. Eleven disjoint preference cases remain challenge-only. All 528 surface atoms and 20 style profiles are project-authored. Qwen-authored prose, pretrained weights, personal calendars, and conversation logs remain excluded. The pipeline checks accuracy, quantization, act coverage, inventory size, memory, compressed size, teacher presence, and data provenance before overwriting the installed artifact.
...
The generated 5,000-example test split reaches 74.95% mean INT8 head accuracy. The reported quantization reduction is 0.36 percentage points. Protected-fact retention and schema-valid candidate rates remain 100%, and unsupported static factual-literal introduction remains zero.

The tracked 250-example TypeScript fixture reports 38.0% exact top-reference selection, 79.6% reference coverage among five candidates, zero recent exact repeats, and about 11.3 ms warm p95 on the latest development-machine run. Exact current results live in `ml/remindspeak/reports/`.
...
Installed users need neither Python nor a network connection. Electron loads the bundled gzip artifact and runs the TypeScript INT8 kernel on Windows, macOS, and Linux. Artifact metadata is version `0.4.0`; the `remindspeak-v0.1-*` filenames remain compatibility names.
