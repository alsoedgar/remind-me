...
Compact block segmentation remains the single-pass default for dense tables, cards, and ordinary documents. A second automatic-layout pass runs only when the first pass exposes weekday headers and calendar-like day numbers; the richer pass must improve the calendar-grid signal before it is selected. Low-confidence, non-calendar OCR receives bounded 90, 270, and finally 180 degree retries, stopping as soon as a clearly readable calendar result wins. The selected upright canvas becomes both the private high-resolution review image and its navigation thumbnail, so evidence boxes remain aligned without downloading orientation data or an OSD language pack.
