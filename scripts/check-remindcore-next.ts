...
console.log(
  `RemindCore Next: ${(routeAccuracy * 100).toFixed(1)}% route, ${(firstCapabilityAccuracy * 100).toFixed(1)}% first capability, ${(exactSequenceAccuracy * 100).toFixed(1)}% exact sequence; ${(selectivePrecision * 100).toFixed(1)}% selective precision at ${(selectiveCoverage * 100).toFixed(1)}% coverage; p95 ${runtimeMetrics.latencyMs.p95.toFixed(1)}ms.`
)
