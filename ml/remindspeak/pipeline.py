...
ACT_BANKS: dict[str, dict[str, Any]] = {
    ...
    "unsupported": {
        "signatures": [("DETAIL",)],
        "leads": ["", "I can’t safely take that on yet.", "That sits outside my local calendar tools.", "I’m keeping this one bounded."],
        "bodies": [
            "<DETAIL>",
            "The safe boundary here is: <DETAIL>",
            "What I can tell you is <DETAIL>",
            "The local assistant stopped because <DETAIL>",
            "That request is outside the supported path: <DETAIL>",
            "I’m limited here because <DETAIL>",
            "For this request, <DETAIL>",
            "The calendar-only answer is <DETAIL>",
            "My supported scope ends here: <DETAIL>",
            "The grounded response is <DETAIL>",
            "I need to stay within this boundary: <DETAIL>",
            "The reason I stopped is: <DETAIL>",
        ],
        "closes": ["", "No calendar data was changed.", "Try phrasing it as a calendar or reminder request.", "I’d rather stop than guess."],
    },
    "error": {
        "signatures": [("DETAIL",)],
...
# Phase 6 gives formerly generic paths their own protected speech acts. The
...
ACT_BANK_EXTENSIONS: dict[str, dict[str, list[str]]] = {
    ...
    "unsupported": {
        "leads": ["I don’t have a safe local action for that yet."],
        "bodies": ["Here’s the honest limitation: <DETAIL>", "What keeps me from acting is <DETAIL>"],
        "closes": ["I can still help if there’s a calendar angle."],
    },
    "error": {
...
def train_and_export(
    ...
    promotion_gates = {
        ...
        "allProtectedSpeechActsCovered": len(SPEECH_ACTS) == 18,
        "projectAtomInventoryComplete": project_authored_surface_atoms == 432,
